...
AccountController.prototype.register = function (newUser, callback){
	...
	me.userModel.findOne({ email: newUser.email }, function(err,user){
		...
		if(err){
			retrn callback(err, new me.ApiResponse({ success: false, extras: { msg: me.ApiMessages.DB_ERROR}}));
		}
		...
		if(user){
			...
			newUser.save(function (err, user, numberAffected){
				...
				if(err) {
					return callback(err, new me.ApiResponse({ sucess: false, extras: { msg: me.ApiMessages.DB_ERROR}}));
				}
